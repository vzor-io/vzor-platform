...
**Что это:** Автономный AI-фреймворк с открытым кодом. Агент работает в Docker-контейнере (Kali Linux), имеет полный root-доступ, выполняет код, ходит в интернет, создаёт субагентов динамически.
...
├── A1 (субагент) — создаётся через call_subordinate
│   ├── A2 (суб-субагент) — A1 может создать своего
│   │   └── A3 ...
│   └── ...
└── ...
```

- **Нет ограничения глубины** — A0→A1→A2→A3→...
- **Один активный субагент** за раз у каждого агента
- `reset: "true"` — новый субагент, `reset: "false"` — продолжить диалог
- `profile: "имя"` — специализированный, `profile: ""` — универсальный (полный доступ)
- Субагент наследует ВСЕ инструменты родителя
