...
**Обновлено:** 2026-02-21 (v5.4 — динамическое создание субагентов, цепочки A0→A1→A2)
...
**Размер:** 565 строк (v5.4)
**Обновлён:** 2026-02-21

Промпт состоит из 10 разделов. Инструменты вызываются через bash (code_execution_tool), кроме call_subordinate — это встроенный tool.
...
7. **Делегируй — создавай субагентов динамически** ★ ОБНОВЛЁН (v5.4): с профилем ИЛИ без (`""` = универсальный). Субагенты могут создавать своих субагентов.
8. Всегда указывай --project
...
**call_subordinate** — динамическое создание субагентов (встроенный tool, НЕ bash) ★ v5.4:

**Как работает:**
- A0 вызывает `call_subordinate` → создаётся субагент A1
- A1 может сам вызвать `call_subordinate` → создаётся A2
- Цепочка: A0 → A1 → A2 → A3... (без ограничения глубины)
- Один активный субагент за раз. `reset: "true"` = новый, `reset: "false"` = продолжить

**Два режима:**

1. **Специализированный** (с профилем):
```json
{
    "tool_name": "call_subordinate",
    "tool_args": {
        "profile": "vzor-analyst",
        "message": "Ты — аналитик проекта zhk-lesnoy-nsk. Задача: ...",
        "reset": "true"
    }
}
```

2. **Универсальный** (без профиля — полный доступ ко всем инструментам):
```json
{
    "tool_name": "call_subordinate",
    "tool_args": {
        "profile": "",
        "message": "Исследуй нормативы по пожарной безопасности для 14-этажного МКД. ...",
        "reset": "true"
    }
}
```

Правила делегирования (v5.4):
| Когда | Кому |
|-------|------|
...
| Исследование, поиск данных, любая задача | `""` (универсальный) |
...
role.md (промпт, 565 строк, v5.4)
  ...
  ├──→ call_subordinate               Встроенный tool → субагенты (v5.4)
  │    ├── profile="vzor-analyst"         Специализированный
  │    ├── profile="vzor-planner"         Специализированный
  │    ├── profile=""                      Универсальный (любая задача)
  │    └── A0→A1→A2→A3...               Цепочки (без лимита глубины)
  │
...
### Обновление 9b: Динамическое создание субагентов (v5.4)
**Причина:** В v5.3 использовались только 4 фиксированных профиля. Agent Zero может создавать субагентов динамически — `profile=""` создаёт универсального субагента с доступом ко всем инструментам. Субагенты могут создавать своих субагентов (цепочки A0→A1→A2→A3).
**Фикс:**
1. **role.md v5.4:** Переписана секция `call_subordinate` — два режима (специализированный + универсальный), цепочки субагентов, расширенная таблица "когда делегировать"
2. **Принцип #7:** Обновлён — "Создавай субагентов динамически через call_subordinate. С профилем или без."
**Результат:** Агент может создавать субагентов для любой задачи, не ограничиваясь 4 профилями.
...
| **v5.4** | **565** | **2026-02-21** | **+ Динамические субагенты: profile="" (универсальный), цепочки A0→A1→A2, обновлён принцип #7** |
...
| Фикс 9b: динамические субагенты | ✔ | profile="" (универсальный), цепочки A0→A1→A2, принцип #7 |
...
├── role_v5.md                           Копия промпта v5.4 (565 строк)
