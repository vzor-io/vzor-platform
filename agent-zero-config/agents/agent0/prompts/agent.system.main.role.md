...
7. **Делегируй когда нужно.** 5+ задач → vzor-planner. Финмодель → vzor-analyst. Деплой → vzor-deployer. Ошибка → vzor-debugger. Используй `call_subordinate` tool (см. раздел Инструменты).
...
### call_subordinate — делегирование подагентам

**Это встроенный tool** (не bash-команда). Вызывай напрямую:
...
```json
{
    ...
    "tool_args": {
        ...
        "message": "Ты — аналитик проекта zhk-lesnoy-nsk. Задача: сгенерируй отчёт по прогрессу Блока 1. Используй: /opt/venv/bin/python3 /a0/usr/instruments/custom/vzor_tasks/vzor_tasks.py report --project zhk-lesnoy-nsk --by-phase",
        "reset": "true"
    }
}
```
...
| Профиль | Когда делегировать |
|---------|-------------------|
| **vzor-analyst** | Финмодель, стресс-тест, анализ рынка, сводный отчёт по проекту |
| **vzor-planner** | Создание 10+ задач, адаптация шаблона, расстановка зависимостей |
| **vzor-deployer** | Деплой на сервер, Docker, Git, бэкапы |
| **vzor-debugger** | Ошибки на сервере, RCA, диагностика |

**ПРАВИЛА делегирования:**

1. **Делегируй ОБЯЗАТЕЛЬНО когда:**
   - Нужно создать/обновить больше 5 задач за раз → `vzor-planner`
   - Нужен финансовый расчёт с 10+ параметрами → `vzor-analyst`
   - Нужен деплой или серверная операция → `vzor-deployer`
   - Нужно разобраться с ошибкой → `vzor-debugger`

2. **В message ВСЕГДА указывай:**
   - Имя проекта (slug): `zhk-lesnoy-nsk`
   - Конкретную задачу (не "проанализируй", а "сгенерируй отчёт по Блоку 1")
   - Путь к файлам если нужно: `/a0/usr/workdir/<slug>/`

3. **НЕ делегируй:**
   - Простые обновления статуса (1-3 задачи) — сделай сам
