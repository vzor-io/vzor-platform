...
# [VZOR-DIRECT-FIX] Импорт прямого клиента Gemini
...
# aggressive environment cleaning to prevent Vertex AI misrouting
for var in list(os.environ.keys()):
    if (var.startswith("GOOGLE_") or var.startswith("GCP_") or var.startswith("CLOUD_")) and "API_KEY" not in var:
        del os.environ[var]

# Force Google AI Studio (LiteLLM)
os.environ["LITELLM_FORCE_GOOGLE_AI_STUDIO"] = "True"
litellm.set_verbose = False
litellm.vertex_ai = False
litellm.vertex_ai_beta = False
litellm.drop_params = True
litellm.custom_provider_map = [] # Clear any custom mappings

# [VZOR-CRITICAL-FIX] Настройка SOCKS5h прокси для обхода утечек IP
proxy = "socks5h://127.0.0.1:12334"
os.environ["HTTP_PROXY"] = proxy
os.environ["HTTPS_PROXY"] = proxy
litellm.proxy_config = {"http": proxy, "https": proxy}
...
class LiteLLMChatWrapper(SimpleChatModel):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # [VZOR-CRITICAL-FIX] Маршрутизация через OpenRouter (приоритет) или Google AI Studio
        openrouter_key = os.getenv("OPENROUTER_API_KEY")
        
        if "gemini-" in model and openrouter_key:
            # Маршрут через OpenRouter
            model_value = f"openrouter/google/{model}"
            kwargs["api_key"] = openrouter_key
            kwargs["extra_headers"] = {
                "HTTP-Referer": "https://vzor.ai",
                "X-Title": "VZOR Agent",
            }
        elif "gemini-" in model:
            # [VZOR-FIX] Нативный маршрут через Google AI Studio
            # Сохраняем ЧИСТОЕ имя модели. Префиксы добавим в unified_call
            model_value = model.split("/")[-1].strip()
            # Убираем параметры Vertex AI
            kwargs.pop("api_base", None)
            kwargs.pop("custom_llm_provider", None)
        elif "/" in model:
            model_value = model
        else:
            model_value = f"{provider}/{model}" if provider not in ["openai", "google", "gemini"] else model
    ...
    async def unified_call(
        ...
        **kwargs: Any,
        ...
        # [VZOR-DIRECT-FIX] Прямой вызов Gemini API (обход LiteLLM)
        use_direct_gemini = "gemini-" in self.model_name and "openrouter" not in self.model_name
        
        if use_direct_gemini:
            print(f"DEBUG: VZOR - Using DIRECT Gemini API call for model={self.model_name}")
            
            # TUN режим - БЕЗ прокси (VPN маскирует весь трафик)
            gemini_client = get_gemini_client(proxy=None)
            
            # Используем чистое имя модели для v1 API
            ...
            while True:
                try:
                    if stream:
                        # Стриминговый вызов
                        async for text_chunk in gemini_client.generate_content_stream(
                            model=clean_model,
                            messages=msgs_conv,
                            temperature=call_kwargs.get("temperature", 0.7),
                            max_tokens=call_kwargs.get("max_tokens", 8192),
                        ):
                            # Формируем чанк в формате LiteLLM
                            parsed = {"response_delta": text_chunk, "reasoning_delta": ""}
                        ...
                        # Не-стриминговый вызов
                        full_text = await gemini_client.generate_content(
                            model=self.model_name,
                            messages=msgs_conv,
                            temperature=call_kwargs.get("temperature", 0.7),
                            max_tokens=call_kwargs.get("max_tokens", 8192),
                        )
                        parsed = {"response_delta": full_text, "reasoning_delta": ""}
                    ...
                    if attempt >= max_retries:
                        raise Exception(f"Direct Gemini API failed after {max_retries} attempts: {e}")
                    
                    print(f"VZOR: Direct Gemini attempt {attempt} failed, retrying...")
                    await asyncio.sleep(retry_delay_s)
        
        # [VZOR] Стандартный путь через LiteLLM (для OpenRouter и других провайдеров)
        attempt = 0
        while True:
            got_any_chunk = False
            try:
                if attempt == 0:
                    print(f"DEBUG: VZOR Routing - Calling acompletion with: model={self.model_name}")
...
class LiteLLMEmbeddingWrapper(Embeddings):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # [VZOR-CRITICAL-FIX] Маршрутизация через OpenRouter (приоритет)
        openrouter_key = os.getenv("OPENROUTER_API_KEY")
        
        if ("gemini-" in model or "text-embedding-" in model) and openrouter_key:
            self.model_name = f"openrouter/google/{model}"
            kwargs["api_key"] = openrouter_key
            kwargs["extra_headers"] = {
                "HTTP-Referer": "https://vzor.ai",
                "X-Title": "VZOR Agent",
            ...
            clean_name = model.strip().split('/')[-1]
            self.model_name = f"gemini/{clean_name}"
            kwargs.pop("api_base", None)
            kwargs.pop("custom_llm_provider", None)
        elif "/" in model or (provider in ["google", "gemini"] and (model.startswith("gemini-") or model.startswith("text-") or model.startswith("embedding-"))):
            self.model_name = model
        else:
            self.model_name = f"{provider}/{model}" if provider not in ["openai", "google"] else model
        self.kwargs = kwargs
        self.a0_model_conf = model_config
...
def get_chat_model(
    ...
    print(f"DEBUG: get_chat_model(provider={provider}, name={name})")
    orig = provider.lower()
...
def get_embedding_model(
    ...
    print(f"DEBUG: get_embedding_model(provider={provider}, name={name})")
    orig = provider.lower()
