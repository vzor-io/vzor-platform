...
**Главное:** Диагностика и починка парсинга 112 документов ПИК, добавленных в 3 новых базы знаний.

### 1. Диагностика проблемы
- **Симптом:** 3 новых базы (Поставщики 5 docs, Технология 28 docs, Экономика 135 docs) — 0 чанков, все документы в очереди 1.5+ часа
- **Причина:** RAPTOR + GraphRAG + DeepDOC включены → каждый чанк требует LLM-вызов → при CPU-режиме TEI это нереально медленно
- **Сервер:** 30/31 GB RAM занято, TEI (BAAI/bge-m3) потребляет 18.79 GB RAM и 254% CPU
...
- **RAGFlow перезапущен** несколько раз (`docker compose restart ragflow-cpu`)

### 3. Текущее состояние (на конец сессии)
| База | ID | Документы | Чанки | Статус |
|------|----|-----------|-------|--------|
| Поставщики | 8b0ee65e0cbf11f18e08d293515dc461 | 5 | 0 | RUNNING (парсится) |
| Технология | 8b0554570cbf11f184c9d293515dc461 | 28 | 0 | RUNNING (парсится) |
| Экономика | 6e79a93a0cbf11f1a7b1d293515dc461 | 79 | 0 | CANCEL (застряла) |
...
- **Экономика (79 docs)** застряла в статусе CANCEL. API не даёт ни остановить, ни перезапустить
- **Решение:** сбросить статус через MySQL: `UPDATE document SET run=0, progress=0 WHERE kb_id='6e79a93a...' AND run=2`
- **Парсинг Поставщики/Технология** — запущен, нужно дождаться завершения
